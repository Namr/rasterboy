...
fn test_matrix_mul() {
    ...
    assert_eq!(tp, Vector3{x: 2.0, y: 2.0, z: 2.0});
}
...
fn test_point_transformations() {
    ...
    let tp = t * p; 
    assert_eq!(tp, Vector3{x: 1.0, y: 1.0, z: 1.0});

    let t = Mat4::from_euler_angles(0.0, 0.0, 90_f32.to_radians());
    let p = Vector3{x: 1.0, y: 0.0, z: 0.0};
    let tp = t * p;
